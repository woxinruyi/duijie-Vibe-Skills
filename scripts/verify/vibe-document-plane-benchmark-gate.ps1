param(
    ...
    function Write-GateArtifacts {
        ...
        $jsonPath = Join-Path $dir 'vibe-document-plane-benchmark-gate.json'
        $mdPath = Join-Path $dir 'vibe-document-plane-benchmark-gate.md'
        Write-VgoUtf8NoBomText -Path $jsonPath -Content ($Artifact | ConvertTo-Json -Depth 100)

        $lines = @(
            '# VCO Document Plane Benchmark Gate',
            '',
    ...
    $artifact = [pscustomobject]@{
        gate = 'vibe-document-plane-benchmark-gate'
        repo_root = $repoRoot
