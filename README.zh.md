...
安装后，只需描述你的任务——路由器自动从 340+ 技能中匹配最合适的：

```
我希望你帮我写一个基因组分析流程 /vibe
→ 路由识别意图 → 自动调度 biopython · scanpy · pysam 等技能
```

**技能冲突、别名重复**——统一由路由层解决，无需手动干预。

### 自定义：接入你自己的 Skill

→ [自定义工作流与 Skill 接入指南](./docs/install/custom-workflow-onboarding.md)
