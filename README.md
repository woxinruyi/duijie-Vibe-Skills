...
`vibe` 现在对用户公开的是一个统一的 governed runtime 入口，不是让用户手动选择 `M`、`L` 或 `XL` 三个档位。

- `M`：窄范围执行，通常是单代理或边界很清楚的小范围工作
- `L`：需要设计、计划、评审和受控子代理协作的中等复杂任务
- `XL`：适合可并行、长流程、需要多代理分波次推进的大任务

这三个级别仍然存在，但它们是 **内部执行级别**。系统会在 `deep_interview` 之后、`plan_execute` 之前，根据任务复杂度、并行性和治理需求自动选择。

默认情况下，路由会 **自动判断** 该走 `M`、`L` 还是 `XL`。

- 如果你什么都不指定，系统会按任务内容自动做内部级别判断
- 一般可以粗略理解为：`L` 更克制、通常更省 token；`XL` 更偏向“用更多 token 换并行度和执行时间”

这也是为什么现在 README 里会看到统一的 `vibe` 入口，同时又会提到 `M/L/XL`：

- 对用户来说：显式调用 `$vibe` 或 `/vibe`，进入同一个治理运行时
- 对系统来说：在同一个运行时里，内部再决定该走 `M`、`L` 还是 `XL`

你也可以在请求里 **显式表达级别偏好**，例如：
...
如果你希望任务明确进入 VibeSkills 的治理流程，可以直接在消息里显式调用：

- Codex：在请求末尾加上 `$vibe`
- Claude Code：在请求末尾加上 `/vibe`

例如：

```text
我希望你先澄清需求，再给出计划并执行实现 $vibe
```
