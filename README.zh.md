...
_Skills 越积越多——但你不需要逐个管理它们。_

### 公开拓扑：一个可见运行时，内部承载完整能力

默认安装现在刻意保持外部公开面很窄：

- 对宿主公开可见的运行时入口：`<target-root>/skills/vibe`
- 内部 bundled 专项 skill 语料：`<target-root>/skills/vibe/bundled/skills/*`
- 兼容性投影：仅在宿主确有需要时，按 host 显式、可测试地投影

这就是这次结构调整的核心：**能力宽度放在内部，公开可见面保持最小化**。`full` 安装仍然保留丰富的路由与专项能力，但默认不再为了让宿主发现 `vibe`，就在 `<target-root>/skills/` 下堆出 300+ 个并列目录。
...
这样可以把宿主安装状态与受管工作区/运行时产物分离，同时保留现有的相对路径运行时契约；如有需要，仍可通过显式 `ArtifactRoot` 覆盖到其他产物目录。
...
| **同一入口内的公开版本** | `全量版本 + 可自定义添加治理` / `仅核心框架 + 可自定义添加治理` |
| **结果** | 在一个地方选择宿主 + 动作 + 版本，然后复制对应提示词 |
...
→ [自定义工作流与 Skill 接入指南](https://github.com/foryourhealth111-pixel/Vibe-Skills/blob/main/docs/install/custom-workflow-onboarding.md)
...
> 💡 **小贴士**：如果你希望每一轮对话都保持在 VibeSkills 的完整受管工作流中，请在每条消息末尾附上 `$vibe` 或 `/vibe`。没有显式附上调用语法的消息，将被视为普通请求而非受管工作流的一部分。

> MCP 说明：`$vibe` 或 `/vibe` 只代表 governed runtime 入口，**不等于 MCP 完成**。repo template、manifest、sidecar，以及 PATH 上已有命令，也都不能单独算某个 MCP 已安装到宿主原生 MCP 配置面。

**当前公开支持面**：`codex`（最强 governed lane）· `claude-code`（支持的安装与使用路径，带 bounded managed closure）· `cursor`（preview-guidance 路径）· `windsurf`（runtime-core 路径，保留 sidecar host-adapter state）· `openclaw`（preview runtime-core adapter 路径）· `opencode`（preview-guidance adapter 路径，public docs 仍保留更薄的 direct install/check）
